    ...
    return (
        <div className={`overflow-y-scroll h-full scrollbar-hide ${MobileView && 'overflow-x-hidden'}`}>
            ...
            <div className="w-full h-full mt-5">
                <div
                    className="w-full bg-cover lg:bg-cover rounded-4xl flex flex-col lg:flex-row"
                    style={{ backgroundImage: `url(${bg})` }}
                >
                    {/* Left Section */}
                    <div className="flex flex-col w-full lg:w-8/12 p-4 gap-4">
                        <div className="flex lg:relative lg:-top-5 sm:flex-row items-center justify-between gap-2">
                            {role !== "employee" && (

                                <div className='bg-[#7697A0] hover:bg-[#4A7079] p-2 h-10 w-10 rounded-full flex justify-center items-center cursor-pointer' onClick={toback}>
                            ...
                            <div className='flex items-center justify-center'>
                                ...
                                <div className="bg-[#7697A0] px-4 py-2 rounded-lg font-bold text-white text-md cursor-pointer" onClick={() => handleDownloadMonth(choseDate, employer?.uuid)}>Generate</div>
                            </div>
                        </div>

                        <div className="flex flex-col gap-4">
                            {/* Latest Payslip */}
                            <div className="w-full min-h-52 bg-[#EAEBE8] rounded-2xl p-2 mt-8 sm:mt-6 lg:mt-0">
                                <EmployerPrevSlip payslip={len ? employer?.payroll_slip?.[len - 1] : undefined} />
                            </div>

                            {/* Previous Payslips Table */}
                            <div className="w-full md:h-[18rem] lg:h-[15rem] bg-[#EAEBE8] rounded-2xl p-2 overflow-y-scroll scrollbar-hide">
                                <h1 style={{ ...FONTS.payroll_head, color: COLORS.primary }} className="pt-2">Previous PaySlips</h1>
                                            ...
                                            {employer?.payroll_slip && employer?.payroll_slip?.length > 0 ? employer?.payroll_slip?.map((items: any, index: any) => (
                                                <tr key={index} style={{ color: COLORS.primary }} className="bg-[#F8F8F8] rounded-lg">
                                                    ...
                                                    <td style={{ ...FONTS.table_data }} className="px-4 py-3 rounded-r-lg">
                                                        <img src={DownloadIcon} alt="" className="w-6 h-6 cursor-pointer" onClick={() => handleDownload(items?.uuid)} />
                                                    </td>
                                                </tr>
                                            )) : (
                                                <tr style={{ color: COLORS.primary }} className="bg-[#F8F8F8] rounded-lg">
                                                    <td style={{ ...FONTS.table_data }} className="px-4 py-3 text-center" colSpan={4}>No Slip Founded</td>
                                                </tr>
                                            )}
                                        </tbody>
                                    </table>
                    ...
                    {/* Right Section */}
                    <div className="w-full lg:w-4/12 p-4 mt-4 lg:mt-0">
                        <div className="bg-[#EAEBE8] rounded-3xl p-4 overflow-auto scrollbar-hide">
                            {/* Profile Header */}
                            <section className='flex sm:flex-row justify-between items-center mb-4 gap-4'>
                                <div className='flex gap-4 items-center'>
                                    <div className='bg-[#DDDED9] text-[#4A7079] h-20 w-20 rounded-xl flex justify-center items-center' style={{ ...FONTS.card_initial }}>{employer?.first_name?.[0]?.toUpperCase()}</div>
                                ...
                                <div className='flex items-center gap-2 bg-[#7697A0] py-2 px-3 rounded-lg text-white cursor-pointer' onClick={() => {
                                    setIsModalOpen(true);
                                    setEdits(employer)
                                }}>
                            ...
                            <div className='border border-[#C3C7C6] my-2'></div>
                            ...
                            <section className='my-2'>
                                ...
                                <div className='grid grid-cols-2 gap-2 mt-2'>
                                    ...
                                    <p style={{ ...FONTS.Nav, color: COLORS.primary }} className='break-words'>{employer?.contact_info?.address}</p>
                                </div>
                            </section>
                        </div>
                    </div>
